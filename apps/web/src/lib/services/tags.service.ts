...
import { invalidateRedisPattern } from '@/lib/cache/cache-utils';
...
export class TagsService {
  ...
  async addEntityTag(input: AddEntityTagInput) {
    ...
    if (!tag) {
      throw new TRPCError({
        code: 'NOT_FOUND',
        message: 'Tag not found or does not belong to tenant',
      });
    }
  ...
  async getEntityTags(input: GetEntityTagsInput) {
    ...
    if (!validated.includeMetadata) {
      ...
      await redis.setex(cacheKey, 300, JSON.stringify(simpleTags));
    }
  ...
  async findEntitiesByTags(input: FindEntitiesByTagsInput) {
    ...
    if (validated.matchAll) {
      ...
      const entityIds = await prisma.$queryRaw<{ entityId: string }[]>`
        SELECT "entityId"
        FROM "EntityTag"
  ...
  async bulkAddTags(
    ...
    if (tags.length !== tagIds.length) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'Some tags not found or do not belong to tenant',
      });
    }
  ...
  private async invalidateTagCache(tenantId: number) {
    await invalidateRedisPattern(`tenant:${tenantId}:tags:*`);
  }
  
  private async invalidateEntityTagCache(entityType: string, entityId: string) {
    await redis.del(`entity:${entityType}:${entityId}:tags`);
  }
}
