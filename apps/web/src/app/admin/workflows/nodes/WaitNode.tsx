...
export function WaitNode({ data, selected }: WaitNodeProps) {
  return (
    <div className={`px-3 py-2 shadow-lg rounded-lg bg-cyan-50 border-2 w-[140px] transition-all duration-200 ${
      selected ? 'border-blue-500 shadow-xl scale-105' : 'border-cyan-500'
    }`}>
      {/* Top input handle */}
      <Handle
        type="target"
