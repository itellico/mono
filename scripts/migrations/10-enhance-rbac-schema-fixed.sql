-- ============================================================================
-- Enhanced RBAC Schema for NestJS Best Practices (Fixed Version)
-- Purpose: Create proper Permission and Role schemas with NestJS Guard support
-- Date: 2024-12-11
-- Prerequisite: Run 09-normalize-table-naming-fixed.sql first
-- ============================================================================
...
-- Add constraints for permissions table
ALTER TABLE permissions 
ADD CONSTRAINT IF NOT EXISTS permissions_name_length 
    CHECK (length(name) >= 3 AND length(name) <= 100),
ADD CONSTRAINT IF NOT EXISTS permissions_pattern_length 
    CHECK (pattern IS NULL OR length(pattern) <= 200),
ADD CONSTRAINT IF NOT EXISTS permissions_priority_range 
    CHECK (priority >= 0 AND priority <= 1000);

-- Add constraints for roles table  
ALTER TABLE roles 
ADD CONSTRAINT IF NOT EXISTS roles_name_length 
    CHECK (length(name) >= 2 AND length(name) <= 100),
ADD CONSTRAINT IF NOT EXISTS roles_code_length 
    CHECK (length(code) >= 2 AND length(code) <= 50),
ADD CONSTRAINT IF NOT EXISTS roles_level_range 
    CHECK (level >= 0 AND level <= 4);

-- Prevent circular inheritance for roles
ALTER TABLE roles 
ADD CONSTRAINT IF NOT EXISTS roles_no_self_inheritance 
    CHECK (id != inherit_from);

-- ============================================================================
-- PHASE 3: ADD FOREIGN KEY CONSTRAINTS
-- ============================================================================

-- Add foreign keys for permissions table
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'users') THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'permissions_created_by_fkey'
        ) THEN
            ALTER TABLE permissions ADD CONSTRAINT permissions_created_by_fkey 
                FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL;
        END IF;
        
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'permissions_updated_by_fkey'
        ) THEN
            ALTER TABLE permissions ADD CONSTRAINT permissions_updated_by_fkey 
                FOREIGN KEY (updated_by) REFERENCES users(id) ON DELETE SET NULL;
        END IF;
        
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'permissions_deleted_by_fkey'
        ) THEN
            ALTER TABLE permissions ADD CONSTRAINT permissions_deleted_by_fkey 
                FOREIGN KEY (deleted_by) REFERENCES users(id) ON DELETE SET NULL;
        END IF;
    END IF;
END $$;

-- Add foreign keys for roles table
DO $$
BEGIN
    -- Tenant foreign key for roles
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'tenants') THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
    ...
    -- Role inheritance foreign key
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'roles_inherit_from_fkey'
    ) THEN
        ALTER TABLE roles ADD CONSTRAINT roles_inherit_from_fkey 
            FOREIGN KEY (inherit_from) REFERENCES roles(id) ON DELETE SET NULL;
    END IF;

    -- User foreign keys for roles
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'users') THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
        ...
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'roles_updated_by_fkey'
        ) THEN
            ALTER TABLE roles ADD CONSTRAINT roles_updated_by_fkey 
                FOREIGN KEY (updated_by) REFERENCES users(id) ON DELETE SET NULL;
        END IF;
        
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'roles_deleted_by_fkey'
        ) THEN
            ALTER TABLE roles ADD CONSTRAINT roles_deleted_by_fkey 
                FOREIGN KEY (deleted_by) REFERENCES users(id) ON DELETE SET NULL;
        END IF;
    END IF;
END $$;
...
-- ============================================================================
-- PHASE 5: ADD SYSTEM PERMISSIONS (with proper updated_at)
-- ============================================================================

-- Insert system permissions with explicit updated_at values
...
-- Insert system roles with explicit updated_at values
...
-- ============================================================================
-- PHASE 6: CREATE HELPER FUNCTIONS FOR NESTJS
-- ============================================================================

-- Function to get user permissions with role inheritance
...
BEGIN
    ...
    RAISE NOTICE 'Enhanced with: constraints, indexes, functions';
END $$;
