...
/**
 * Auto-migrated route: queue
 * Tier: tenant
 * Permission: tenant.queue.manage
 */
export const queueRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.queue.manage')
    ],
    schema: {
      tags: ['tenant.queue'],
      security: [{ bearerAuth: [] }],
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/queue/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in queue route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in queue POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
