...
/**
 * Auto-migrated route: translations
 * Tier: tenant
 * Permission: tenant.translations.manage
 */
export const translationsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.translations.manage')
    ],
    schema: {
      tags: ['tenant.translations'],
      security: [{ bearerAuth: [] }],
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/translations/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in translations route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in translations POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
