...
/**
 * Auto-migrated route: permissions
 * Tier: tenant
 * Permission: tenant.permissions.manage
 */
export const permissionsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.permissions.manage')
    ],
    schema: {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/permissions/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in permissions route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in permissions POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
