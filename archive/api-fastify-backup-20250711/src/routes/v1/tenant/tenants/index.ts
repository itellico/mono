...
/**
 * Auto-migrated route: tenants
 * Tier: tenant
 * Permission: tenant.tenants.manage
 */
export const tenantsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.tenants.manage')
    ],
    schema: {
      tags: ['tenant.tenants'],
      security: [{ bearerAuth: [] }],
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/tenants/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in tenants route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in tenants POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
