...
/**
 * Auto-migrated route: preferences
 * Tier: tenant
 * Permission: tenant.preferences.manage
 */
export const preferencesRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.preferences.manage')
    ],
    schema: {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/preferences/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in preferences route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in preferences POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
