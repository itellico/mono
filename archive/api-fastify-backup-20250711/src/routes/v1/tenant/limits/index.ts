...
/**
 * Auto-migrated route: limits
 * Tier: tenant
 * Permission: tenant.limits.manage
 */
export const limitsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.limits.manage')
    ],
    schema: {
      tags: ['tenant.limits'],
      security: [{ bearerAuth: [] }],
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/limits/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in limits route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in limits POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
