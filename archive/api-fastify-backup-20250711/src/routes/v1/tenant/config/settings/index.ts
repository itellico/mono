 ...
 */
export const config_settingsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.config.settings.manage')
    ],
    schema: {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/settings/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in config_settings route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in config_settings POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
...
export default config_settingsRoutes;
