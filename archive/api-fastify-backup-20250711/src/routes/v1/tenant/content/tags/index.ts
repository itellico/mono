...
/**
 * Manage tenant content tags
 * Tier: tenant
 * Permission: tenant.content.tags.manage
 */
export const content_tagsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.content.tags.manage')
    ],
    schema: {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/tags/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in content_tags route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in content_tags POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
...
export default content_tagsRoutes;
