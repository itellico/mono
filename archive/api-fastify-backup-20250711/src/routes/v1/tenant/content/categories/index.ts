...
/**
 * Manage tenant content categories
 * Tier: tenant
...
export const content_categoriesRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/categories/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in content_categories route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in content_categories POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
...
export default content_categoriesRoutes;
