...
/**
 * Auto-migrated route: versions
 * Tier: tenant
 * Permission: tenant.versions.manage
 */
export const versionsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.versions.manage')
    ],
    schema: {
      tags: ['tenant.versions'],
      security: [{ bearerAuth: [] }],
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/versions/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in versions route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in versions POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
