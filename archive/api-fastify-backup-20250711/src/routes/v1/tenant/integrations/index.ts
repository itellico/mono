...
/**
 * Auto-migrated route: integrations
 * Tier: tenant
 * Permission: tenant.integrations.manage
 */
export const integrationsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.integrations.manage')
    ],
    schema: {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/integrations/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in integrations route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in integrations POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
