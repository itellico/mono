...
/**
 * Auto-migrated route: modules
 * Tier: tenant
 * Permission: tenant.modules.manage
 */
export const modulesRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.modules.manage')
    ],
    schema: {
      tags: ['tenant.modules'],
      security: [{ bearerAuth: [] }],
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/admin/modules/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in modules route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in modules POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
