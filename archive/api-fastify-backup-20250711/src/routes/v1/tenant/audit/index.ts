...
/**
 * Auto-migrated route: audit
 * Tier: tenant
 * Permission: tenant.audit.manage
 */
export const auditRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    preHandler: [
      ...
      fastify.requirePermission('tenant.audit.manage')
    ],
    schema: {
      tags: ['tenant.audit'],
      security: [{ bearerAuth: [] }],
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          originalPath: 'src/app/api/v1/audit/route.ts'
        }
      };
    } catch (error) {
      fastify.log.error('Error in audit route:', error);
  ...
  fastify.post('/', {
    ...
    try {
      ...
      fastify.log.error('Error in audit POST route:', error);
      return reply.code(500).send({
        success: false,
        error: 'INTERNAL_SERVER_ERROR',
        message: 'Internal server error'
